...
- **Killed by new tests:** 13 (see `techniques_advanced_test.go`)
- **Annotated equivalents:** 0 (no genuine equivalents identified)
- **Deferred (test gaps, NOT equivalent):** 5
...
(12 rows cover 13 mutants because `TestCheckChainEliminationSkipsRCDigit`
...
- `als_chains.go:129` `numbers/decrementer` (`FindAllALS(b, 4)` -> `3`): needs a
  fixture whose only firing chain contains a size-4 ALS. All constructible
  chains fire on size-1/2 ALS, which `maxSize=3` still enumerates.
- `als_chains.go:158` `numbers/decrementer` (chain `maxLen` 6 -> 5): needs a
