...
export class SudokuGameSession {
  ...
  async startGame(difficulty: 'easy' | 'medium' | 'hard' | 'expert'): Promise<ActionResult> {
    ...
    try {
      ...
      const button = this.page
        .locator(`button:has-text("${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}")`)
        .first()
      await button.click()
  ...
  async getGameState(): Promise<GameState> {
    ...
    const isComplete = await this.page
      .locator('text=/Completed|Congratulations/i')
      .isVisible()
      .catch(() => false)
  ...
  async clickCell(index: number): Promise<ActionResult> {
    ...
    try {
      ...
      return {
        success: false,
        message: `Failed to interact with cell ${index}`,
        error: String(error),
      }
    }
  }
  ...
  async openMenu(): Promise<ActionResult> {
    ...
    try {
      ...
      await this.page.waitForSelector('button:has-text("Validate"), button:has-text("Solve")', {
        timeout: 2000,
      })
