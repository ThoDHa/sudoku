...
async function openMenu(page: Page): Promise<void> {
  const menuButton = page
    .locator('button[aria-label="Menu"], button:has-text("Menu"), [data-testid="menu-button"]')
    .first()
...
test.describe('@slow Hint Consistency - Technique and Regular Hints Match', () => {
  ...
  test('technique hint and regular hint work together after autofill', async ({ page }) => {
    ...
    while (iterationCount < maxIterations) {
      ...
      console.log(
        `Iteration ${iterationCount + 1}: Technique="${techniqueName}", Cell=${cellRef}, Digit=${digit}`,
      )
