...
const HINT_THRESHOLDS = {
  ...
  SUBSEQUENT_HINT_MAX_MS: 1500, // max single subsequent hint (noise ceiling)
  // Steady-state hints are very fast (~100ms cached), so a single-sample ratio
  // is dominated by run noise. The drift guard compares 3-sample half-averages
  // with a 2.0x tolerance: catches gradual degradation without flaking on the
  // sub-10ms jitter that breaks a tight single-sample ratio.
  NO_DRIFT_RATIO: 2.0,
...
test.describe.serial('@profiling Hint / Autosolve Latency', () => {
  test('hint latency stays bounded across first (cold) and subsequent (cached) hints', async ({ page }) => {
    ...
    // No drift across the steady-state sequence: the second half's average
    // must not be dramatically slower than the first half's (catches
    // accumulating state, toast queueing, or cache degradation). Uses
    // half-averages rather than a single-sample ratio because cached hints are
    // fast enough (~100ms) that single-sample ratios are noise-dominated.
    ...
    console.log(
      `Hint drift — first-${half} avg ${firstHalf.avg.toFixed(2)}ms, ` +
      `last-${half} avg ${secondHalf.avg.toFixed(2)}ms ` +
      `(ratio ${(secondHalf.avg / firstHalf.avg).toFixed(2)}x)`
    );
    expect(secondHalf.avg).toBeLessThan(firstHalf.avg * HINT_THRESHOLDS.NO_DRIFT_RATIO);
  });
});
