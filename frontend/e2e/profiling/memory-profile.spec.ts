...
test.describe.serial('@profiling Memory - Light Profiling', () => {
  ...
  test('page navigation cycles do not leak memory (10 cycles)', async ({ page }) => {
    ...
    for (let cycle = 0; cycle < 10; cycle++) {
      ...
      console.log(
        `📊 Cycle ${cycle + 1}: heap=${(cycleMetrics.jsHeapUsedSize / 1024 / 1024).toFixed(2)}MB, nodes=${cycleMetrics.nodes}`,
      )
    }
  ...
  test('auto-solve loop memory stays bounded (5 puzzles)', async ({ page }) => {
    ...
    for (let i = 0; i < puzzleSeeds.length; i++) {
      ...
      try {
        ...
        if (await autoSolveButton.isVisible({ timeout: 2000 })) {
          ...
          const solveCompleteIndicator = page.locator(
            '[data-testid="puzzle-complete"], .puzzle-complete, .game-won',
          )
          try {
        ...
        console.log(
          `📊 Auto-solve button not found for puzzle ${i + 1}, making manual moves instead`,
        )
      ...
      console.log(
        `📊 After puzzle ${i + 1}: heap=${(puzzleMetrics.jsHeapUsedSize / 1024 / 1024).toFixed(2)}MB`,
      )
    }
    ...
    await cdp.forceGC()
    const finalMetrics = await cdp.getMemoryMetrics()
    const totalGrowthMB = calculateGrowthMB(
      initialMetrics.jsHeapUsedSize,
      finalMetrics.jsHeapUsedSize,
    )
...
test.describe.serial('@profiling @slow Memory - Deep Profiling', () => {
  test.skip(
    ({ browserName }) => browserName !== 'chromium',
    'CDP heap profiling only works in Chromium',
  )
  ...
  test('heap snapshot comparison before/after gameplay', async ({ page }) => {
    ...
    console.log(
      `📊 Initial heap (performance.memory): ${initialMemory ? (initialMemory.usedJSHeapSize / 1024 / 1024).toFixed(2) : 'N/A'} MB`,
    )

    const initialMetrics = await cdp.getMemoryMetrics()
    console.log(
      `📊 Initial heap (page.metrics): ${(initialMetrics.jsHeapUsedSize / 1024 / 1024).toFixed(2)} MB`,
    )
    ...
    console.log(
      `📊 Final heap (performance.memory): ${finalMemory ? (finalMemory.usedJSHeapSize / 1024 / 1024).toFixed(2) : 'N/A'} MB`,
    )

    const finalMetrics = await cdp.getMemoryMetrics()
    console.log(
      `📊 Final heap (page.metrics): ${(finalMetrics.jsHeapUsedSize / 1024 / 1024).toFixed(2)} MB`,
    )

    const metricsGrowthMB = calculateGrowthMB(
      initialMetrics.jsHeapUsedSize,
      finalMetrics.jsHeapUsedSize,
    )
    ...
    if (initialMemory && finalMemory) {
      const perfGrowthMB = calculateGrowthMB(
        initialMemory.usedJSHeapSize,
        finalMemory.usedJSHeapSize,
      )
