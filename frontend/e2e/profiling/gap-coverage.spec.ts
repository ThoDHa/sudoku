...
test.describe.serial('@profiling Gap Coverage', () => {
  test.describe('Candidate-Toggle (Notes Mode) Render', () => {
    test('candidate-toggle render stays responsive across many cells', async ({ page }) => {
      ...
      console.log(`Candidate-toggle — avg ${stats.avg.toFixed(2)}ms, p95 ${stats.p95.toFixed(2)}ms, max ${stats.max.toFixed(2)}ms (${count} cells)`);
    ...
    test('candidate marks do not leak DOM nodes', async ({ page }) => {
      ...
      console.log(`Candidate nodes — before ${before}, after ${after}, marks ${marksToggled}, growth/mark ${growthPerMark.toFixed(1)}`);
  ...
  test.describe('History Undo/Redo Throughput', () => {
    test('undo/redo cycle latency stays bounded', async ({ page }) => {
      ...
      console.log(`Undo/Redo — undo ${perUndo.toFixed(2)}ms/op, redo ${perRedo.toFixed(2)}ms/op (${placed} ops each)`);
