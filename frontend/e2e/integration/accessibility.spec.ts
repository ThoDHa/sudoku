...
test.describe('@integration Accessibility - Keyboard Focus', () => {
  ...
  test('unselected cells have tabindex -1', async ({ page }) => {
    ...
    const isSelected = await secondCell.evaluate(
      (el) => el.classList.contains('ring-accent') || el.classList.contains('ring-2'),
    )
...
test.describe('@integration Accessibility - Color Contrast', () => {
  ...
  test('board cells have visible text in light mode', async ({ page }) => {
    ...
    // Get computed color
    const color = await firstFilled.evaluate((el) => window.getComputedStyle(el).color)
...
test.describe('@integration Accessibility - Modals', () => {
  ...
  test('escape key closes menu and preserves cell selection', async ({ page }) => {
    ...
    const emptyCell = page.locator('[role="gridcell"][aria-label*="empty"]').first()
    await emptyCell.click()
    await expect(page.locator('[role="gridcell"][class*="ring-accent"]')).toHaveCount(1, {
      timeout: 5000,
    })
  ...
  test('escape key deselects cell when menu is closed', async ({ page }) => {
    ...
    await expect(page.locator('[role="gridcell"][class*="ring-accent"]')).toHaveCount(0, {
      timeout: 5000,
    })
  })
})
...
test.describe('@integration Accessibility - Button Labels', () => {
  ...
  test('undo and redo buttons have accessible labels', async ({ page }) => {
    ...
    if (await undoButton.isVisible()) {
      const label =
        (await undoButton.getAttribute('aria-label')) || (await undoButton.getAttribute('title'))
  ...
  test('hint button has accessible label', async ({ page }) => {
    ...
    if (await hintButton.isVisible()) {
      const label =
        (await hintButton.getAttribute('aria-label')) || (await hintButton.getAttribute('title'))
