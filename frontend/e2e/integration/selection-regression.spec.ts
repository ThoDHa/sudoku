...
async function getOutsideClickCoordinates(page: any) {
  ...
  return {
    ...
    topRight: paddingRight
      ? { x: viewport.width - paddingRight, y: Math.max(gameBoxY - paddingTop, 10) }
      : null,
    bottomLeft: safeBottomY && paddingLeft ? { x: paddingLeft, y: safeBottomY } : null,
    bottomRight:
      safeBottomY && paddingRight ? { x: viewport.width - paddingRight, y: safeBottomY } : null,
  }
}
      ...
      test.describe('Rapid Interaction Stress Tests', () => {
        ...
        test('maintains correct state during mixed rapid interactions', async ({ page }) => {
          ...
          for (const step of sequence) {
            ...
            if (step.action !== 'arrow') {
              // Arrow results can vary
...
test.describe('@regression Overlay Interaction Selection Preservation', () => {
  ...
  test('dismissing history via backdrop preserves cell selection', async ({ page }) => {
    ...
    await expect(page.locator('[data-modal]:visible')).toBeVisible()
    await page
      .locator('[data-overlay-backdrop]:visible')
      .first()
