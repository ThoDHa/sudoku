 ...
 */
async function useHintsAndVerifyStable(
...
test.describe('@mobile Game Features - Mobile', () => {
  ...
  test('technique hint button is accessible', async ({ page, mobileViewport }) => {
    ...
    const techniqueButton = page
      .locator('button:has-text("Technique"), button:has-text("?")')
      .first()
  ...
  test('technique hint shows toast or modal', async ({ page, mobileViewport }) => {
    ...
    const techniqueButton = page
      .locator('button:has-text("Technique"), button:has-text("?"), button[title*="echnique"]')
      .first()
    ...
    if (await techniqueButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const infoToast = page.locator(
        '.fixed.z-50:has-text("Fill in"), .fixed.z-50:has-text("candidates")',
      )
  ...
  test('technique modal fits within viewport', async ({ page, mobileViewport }) => {
    ...
    if (await techniqueButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const learnMoreButton = page.locator(
        '.fixed.z-50 button:has-text("Learn more"), .fixed.z-50 .underline:has-text("Learn more")',
      )
      const gotItButton = page.getByRole('button', { name: /Got it|Close|OK/i })
...
test.describe('@mobile Touch Interactions', () => {
  ...
  test('erase button clears cell', async ({ page, mobileViewport }) => {
    ...
    const emptyCell = page
      .locator('[role="gridcell"][aria-label*="Row 7"][aria-label*="empty"]')
      .first()
    const ariaLabel = await emptyCell.getAttribute('aria-label')
    const coords = parseAriaLabel(ariaLabel)
    ...
    if (coords) {
      const cellByCoords = page.locator(
        `[role="gridcell"][aria-label*="Row ${coords.row}, Column ${coords.col}"]`,
      )
      await expect(async () => {
...
test.describe('@mobile Touch Target Compliance', () => {
  test('difficulty buttons meet WCAG touch target requirements', async ({
    page,
    mobileViewport,
  }) => {
    void mobileViewport
...
test.describe('@mobile Mobile Keyboard Handling', () => {
  test('native virtual keyboard does not appear when tapping cells', async ({
    page,
    mobileViewport,
  }) => {
    void mobileViewport
    ...
    const emptyCell = page
      .locator('[role="gridcell"][aria-label*="Row 4"][aria-label*="empty"]')
      .first()
    await emptyCell.scrollIntoViewIfNeeded()
    await emptyCell.tap()
  ...
  test('physical keyboard input works when focused', async ({ page, mobileViewport }) => {
    ...
    // Select an empty cell
    const emptyCell = page
      .locator('[role="gridcell"][aria-label*="Row 8"][aria-label*="empty"]')
      .first()
    const ariaLabel = await emptyCell.getAttribute('aria-label')
    const coords = parseAriaLabel(ariaLabel)
