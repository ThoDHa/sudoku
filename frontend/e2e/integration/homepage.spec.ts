...
test.describe('Homepage - Mode Switching', () => {
  ...
  test('daily mode persists after starting a daily game', async ({ page }) => {
    ...
    if (!isComplete) {
      ...
      const dailyOrComplete = page
        .locator('h1:has-text("Daily Sudoku")')
        .or(page.locator('h1:has-text("Daily Complete")'))
...
test.describe('Homepage - Continue Game', () => {
  test('shows continue button for in-progress game in game mode', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Continue').or(page.locator('[data-resumable="true"]')).or(easyButton),
    ).toBeVisible()
  })
  ...
  test('new game confirmation modal appears when switching difficulties with in-progress game', async ({
    page,
  }) => {
    const gameSeed = `P${Date.now()}`
    const gameState = {
...
test.describe('Homepage - Daily Mode Continue Game', () => {
  ...
  test('daily mode confirmation modal appears when switching difficulties with in-progress game', async ({
    page,
  }) => {
    const dailySeed = 'daily-2024-12-25'
    const gameState = {
...
test.describe('Homepage - Daily Mode Features', () => {
  ...
  test('daily complete mode shows streak and play button when daily is completed', async ({
    page,
  }) => {
...
test.describe('Homepage - Edge Cases', () => {
  test('fresh user with no localStorage sees default state', async ({ page }) => {
    ...
    expect(
      isGridVisible || (await page.locator('button:has-text("Play")').isVisible()),
    ).toBeTruthy()
  })
  ...
  test('page is accessible via keyboard navigation', async ({ page }) => {
    test.skip(
      ['pixel-5', 'iphone-12'].includes(test.info().project.name),
      'Keyboard navigation not applicable on touch devices',
    )
    await page.goto('/')
    await page.waitForLoadState('networkidle')
    ...
    for (let i = 0; i < 40 && !found; i++) {
      ...
      if (
        activeText.includes('play') ||
        activeText.includes('easy') ||
