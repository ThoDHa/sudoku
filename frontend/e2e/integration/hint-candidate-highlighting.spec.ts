...
test.describe('@integration Hint Candidate Highlighting Bug', () => {
  ...
  test('BUG: only the correct candidate should be highlighted green, not all candidates', async ({
    page,
  }) => {
  ...
  test('multi-hint: trigger many hints to catch the bug in various states', async ({ page }) => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const buggyCells = highlightState.filter(
        (cell) => cell.allCandidates.length > 1 && cell.greenDigits.length > 1,
      )
  ...
  test('immediate check: capture state right after hint click before dismissal', async ({
    page,
  }) => {
    await setupGameAndWaitForBoard(page, {
      seed: 'daily-2026-03-21',
    ...
    const buggyCellsImmediate = immediateHighlight.filter(
      (cell) => cell.allCandidates.length > 1 && cell.greenDigits.length > 1,
    )

    const buggyCellsAfter = afterWaitHighlight.filter(
      (cell) => cell.allCandidates.length > 1 && cell.greenDigits.length > 1,
    )
  ...
  test('check cell background highlighting and candidate highlighting separately', async ({
    page,
  }) => {
    await setupGameAndWaitForBoard(page, {
      seed: 'daily-2026-03-21',
    ...
    const buggyCells = highlightAnalysis.cellsWithGreenCandidates.filter(
      (cell) => cell.allCandidates.length > 1 && cell.greenDigits.length > 1,
    )
