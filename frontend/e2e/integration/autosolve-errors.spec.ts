...
async function findEmptyCellInRow(
  page: Page,
  rowHint: number,
): Promise<{ row: number; col: number } | null> {
  const emptyCell = page
    .locator(`[role="gridcell"][aria-label*="Row ${rowHint}"][aria-label*="empty"]`)
    .first()
...
async function findFilledDigitInRow(page: Page, row: number): Promise<number | null> {
  const filledCells = page.locator(
    `[role="gridcell"][aria-label*="Row ${row}"][aria-label*="value"]`,
  )
...
async function findFilledDigitInColumn(page: Page, col: number): Promise<number | null> {
  const filledCells = page.locator(
    `[role="gridcell"][aria-label*="Column ${col}"][aria-label*="value"]`,
  )
  const count = await filledCells.count()
  if (count === 0) return null
...
async function enterDigitInCell(
  page: Page,
  row: number,
  col: number,
  digit: number,
...
test.describe('@integration Autosolve Error Handling', () => {
  ...
  test('autosolve handles duplicate in row gracefully', async ({ page }) => {
    ...
    for (const row of [5, 4, 6, 3, 7]) {
      const emptyCells = page.locator(
        `[role="gridcell"][aria-label*="Row ${row}"][aria-label*="empty"]`,
      )
  ...
  test('autosolve handles duplicate in column gracefully', async ({ page }) => {
    ...
    for (const col of [5, 4, 6, 3, 7]) {
      const emptyCells = page.locator(
        `[role="gridcell"][aria-label*="Column ${col}"][aria-label*="empty"]`,
      )
      const count = await emptyCells.count()
