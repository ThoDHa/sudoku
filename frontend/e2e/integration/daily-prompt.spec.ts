...
test.describe('Daily Prompt Modal - Appearance Conditions', () => {
  test('shows modal when loading practice game without completing daily', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      console.log(
        'DEBUG: Manual check - localStorage available:',
      ...
      try {
        ...
        console.log(
          'DEBUG: Manual check - All conditions for modal:',
    ...
    await page.evaluate(() => {
      ...
      const modalElements = document.querySelectorAll(
        '[class*="modal"], [role="dialog"], [class*="fixed"]',
      )
      console.log('DEBUG: All modal-like elements:', modalElements.length)
      modalElements.forEach((modal, index) => {
        console.log(
          `DEBUG: modal[${index}]: class="${modal.className}" visible="${getComputedStyle(modal).display}"`,
        )
      })

      // Check for DailyPromptModal specifically
      const dailyPromptElements = document.querySelectorAll('*')
      const dailyTexts = Array.from(dailyPromptElements).filter(
        (el) => el.textContent && el.textContent.includes('Daily'),
      )
...
test.describe('Daily Prompt Modal - Button Functionality', () => {
  test('"Go to Daily" button navigates to today\'s daily puzzle and shows difficulty chooser', async ({
    page,
  }) => {
  ...
  test('"Continue Practice" button closes modal and continues loading practice game', async ({
    page,
  }) => {
    // Set up clean state BEFORE any navigation
    const seed = `P${Date.now()}`
  ...
  test('"Don\'t show this again" checkbox disables future prompts', async ({ page }) => {
    ...
    await expect(async () => {
      const debugBeforeSecondNav = await page.evaluate(() => {
        ...
        if (!rawPrefs) return undefined
        // Preferences are stored in a versioned envelope ({schemaVersion, data});
        // fall back to the legacy flat shape for older entries.
        const parsed = JSON.parse(rawPrefs)
        const prefs = parsed?.data ?? parsed
    ...
    const debugBeforeSecondNav = await page.evaluate(() => {
      const rawPrefs = localStorage.getItem('sudoku_preferences')
      if (!rawPrefs) return { showDailyReminder: undefined }
      const parsed = JSON.parse(rawPrefs)
      const prefs = parsed?.data ?? parsed
      return {
        rawPrefs: rawPrefs,
