...
describe('diffUtils', () => {
  ...
  describe('roundtrip consistency', () => {
    it('apply then unapply returns to original state', () => {
      ...
      // Apply diff
      const { board: appliedBoard, candidates: appliedCandidates } = applyStateDiff(
        originalBoard,
        originalCandidates,
        diff,
      )
      ...
      // Unapply diff
      const { board: restoredBoard, candidates: restoredCandidates } = unapplyStateDiff(
        appliedBoard,
        appliedCandidates,
        diff,
      )
  ...
  describe('memory calculations', () => {
    it('calculates diff memory size correctly', () => {
      const diff: StateDiff = {
        ...
        candidateChanges: [{ idx: 5, oldMask: 0, newMask: 0b1010 }],
      }
