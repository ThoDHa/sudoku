...
interface MediaQueryListStubOptions {
  matches?: boolean
  onAddEventListener?: (event: string, cb: (e: MediaQueryListEvent) => void) => void
  onRemoveEventListener?: (event: string, cb: (e: MediaQueryListEvent) => void) => void
}

// Shared MediaQueryList stub shape: jsdom does not implement matchMedia, so
// every stub in this file builds its list objects here, overriding only the
// behaviour a specific test needs.
function mediaQueryList(
  query: string,
  { matches = false, onAddEventListener, onRemoveEventListener }: MediaQueryListStubOptions = {},
) {
  return {
    matches,
    media: query,
    ...
    addEventListener: onAddEventListener ?? vi.fn(),
    removeEventListener: onRemoveEventListener ?? vi.fn(),
    dispatchEvent: vi.fn(),
  }
}
...
function stubMatchMedia() {
  Object.defineProperty(window, 'matchMedia', {
    writable: true,
    value: vi.fn().mockImplementation((query: string) => mediaQueryList(query)),
  })
}
...
describe('ThemeProvider preference restore', () => {
  ...
  it('resolves the system mode as dark when matchMedia reports dark', () => {
    Object.defineProperty(window, 'matchMedia', {
      ...
      value: vi
        .fn()
        .mockImplementation((query: string) => mediaQueryList(query, { matches: true })),
    })
  ...
  Object.defineProperty(window, 'matchMedia', {
    ...
    value: vi.fn().mockImplementation((query: string) => {
      ...
      return mediaQueryList(query, {
        ...
        onRemoveEventListener: (event, cb) => {
          if (event === 'change') {
...
// Full reset for tests that mount the provider: isolated localStorage, a clean
// documentElement, and a matchMedia stub so the provider mounts in jsdom.
...
describe('ThemeProvider color palette application', () => {
  beforeEach(resetProviderTestState)
