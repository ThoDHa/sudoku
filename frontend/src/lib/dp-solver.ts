...
export function findConflicts(grid: number[]): Conflict[] {
  ...
  const findInUnit = (cellIndices: number[], type: Conflict['type']) => {
    ...
    for (const [val, cells] of positions) {
      emitConflictingPairs(cells, val, type)
    }
  }
  ...
  for (let box = 0; box !== BOARD_SIZE; box++) {
    ...
    const indices = Array.from(
      { length: SUBGRID_SIZE * SUBGRID_SIZE },
      (_, k) => (boxRow + Math.floor(k / SUBGRID_SIZE)) * BOARD_SIZE + boxCol + (k % SUBGRID_SIZE),
    )
...
export function validateBoardAgainstSolution(
  ...
  for (let i = 0; i !== TOTAL_CELLS; i++) {
    // The length guard at the top of this function enforces
    // board.length === solution.length === TOTAL_CELLS, so for every in-range i
    // the ?? 0 fallbacks are unreachable defensive defaults.
 ...
 * Exported so tests can pin the maxCount prune bound directly;
 * hasUniqueSolution only observes count === 1 and cannot distinguish it.
 */
export function countSolutions(grid: number[], maxCount: number): number {
  const board = [...grid]
  let count = 0
