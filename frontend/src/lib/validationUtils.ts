  ...
  forEachUnit((_unitType, _index, cells) => {
    ...
    for (const idx of cells) {
      ...
      if (val === 0 || seen.has(val)) {
        isValid = false
        // Stryker disable next-line BooleanLiteral: the only replacement flips the early-stop to true; forEachUnit treats true like undefined (continue) and isValid is already false, so the final return value is unchanged for every board
