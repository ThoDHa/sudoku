...
export default function GlossaryModal({ isOpen, onClose }: GlossaryModalProps) {
  ...
  return (
    <Dialog
      isOpen={isOpen}
      onClose={onClose}
      titleId="glossary-modal-title"
      panelClassName="max-h-[85vh] w-full max-w-2xl overflow-hidden rounded-2xl bg-background shadow-xl flex flex-col"
      backdropClassName="bg-black/50"
    >
      {/* Header */}
      <div className="flex items-center justify-between border-b border-board-border-light p-4">
        <h2 id="glossary-modal-title" className="text-xl font-bold text-foreground">
          Sudoku Glossary
        </h2>
        <button
          onClick={onClose}
