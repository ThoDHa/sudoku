...
interface MenuProps {
  isOpen: boolean
  onClose: () => void
  // Optional: current game seed for mode detection
...
// =============================================================================
// SUBCOMPONENT: ModeSwitchSection
// =============================================================================
interface ModeSwitchSectionProps {
  onClose: () => void
  currentSeed?: string | undefined // Optional - may not be available on all pages
}

function ModeSwitchSection({ onClose, currentSeed }: ModeSwitchSectionProps) {
  const navigate = useNavigate()

  // Detect current game mode from seed
  const currentMode = currentSeed ? getGameMode(currentSeed) : null

  // Don't show mode switch if we can't determine mode
    ...
    <>
      {currentMode === 'practice' && (
        <button
          ...
          className="flex w-full items-center gap-3 px-3 py-2.5 text-foreground rounded-lg hover:bg-btn-hover"
        >
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path
              strokeLinecap="round"
    ...
    <>
      ...
      <div className="rounded-lg overflow-hidden">
        ...
        >
          ...
          <svg
            className={`h-3 w-3 transition-transform ${newPuzzleMenuOpen ? 'rotate-180' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
  ...
  return (
    <div className="rounded-lg overflow-hidden">
      ...
      >
        ...
        <svg
          className={`h-3 w-3 transition-transform ${settingsExpanded ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        <div className="ml-4 py-1 space-y-1">
          ...
          <div className="flex items-center justify-between px-3 py-2">
            <span className="text-sm text-foreground-muted">
              {COLOR_THEMES.find((t) => t.key === colorTheme)?.label ?? colorTheme}
            </span>
                ...
                <span className="flex items-center gap-3">
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M8 7H5a2 2 0 00-2 2v12a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3-3m0 0l-3 3m3-3v12"
                    />
                  </svg>
                  Auto-Save Progress
                </span>
        ...
        <div className="min-h-full flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-3 space-y-1">
              ...
              {/* Mode Switch Section (for game page) */}
              {gameActions && <ModeSwitchSection onClose={onClose} currentSeed={currentSeed} />}
              ...
              >
                ...
                {debugInfoCopied ? 'Copied!' : 'Copy Debug Info'}
              </button>
              ...
              {/* Instructions */}
              <div className="px-3 py-2 text-xs text-foreground-muted border-t border-board-border-light">
                To report a bug: first copy debug info above, then paste it in your GitHub issue.
              </div>
