...
// cycle for a single digit. Coloring the net leaves two same-colored
// candidates of that digit as peers in the same unit, which is the 3D Medusa
// same-unit contradiction (Rule 2): the whole color is false and every
// candidate of that color is eliminated.
func TestDetectMedusa3DFiresSameUnitContradiction(t *testing.T) {
	...
	//   R4C2<->R2C2 (col 2), R2C2<->R1C1 (box 1). This odd cycle colors
	//   R4C5 and R4C2 with the same color, and they share row 4.
...
// TestCheckSameUnitContradictionEliminatesColor drives checkSameUnitContradiction
// directly: two same-colored candidates of the same digit that are peers form a
// contradiction, so every candidate of that color is eliminated.
func TestCheckSameUnitContradictionEliminatesColor(t *testing.T) {
	b := &testBoard{}
	b.candidates[idxOf(0, 0)] = NewCandidates([]int{5}) // R1C1
	b.candidates[idxOf(0, 1)] = NewCandidates([]int{5}) // R1C2, peer of R1C1 in row 1
	b.candidates[idxOf(0, 8)] = NewCandidates([]int{5}) // R1C9, the opposite color
	...
	if len(move.Eliminations) != 2 {
		t.Errorf("expected both color-1 candidates eliminated, got %+v", move.Eliminations)
	}
	if !strings.Contains(move.Explanation, "same unit") {
...
// TestCheckAllCandidatesSameColorEliminatesColor drives checkAllCandidatesSameColor
// (Rule 6): a cell whose every candidate carries the same color would be empty
// if that color were false, so the color is false and all of it is eliminated.
func TestCheckAllCandidatesSameColorEliminatesColor(t *testing.T) {
	b := &testBoard{}
	b.candidates[idxOf(0, 0)] = NewCandidates([]int{3, 4}) // R1C1: both candidates color 1
	...
	if move == nil {
		t.Fatal("expected an all-candidates-same-color move")
	}
	if move.Action != "eliminate" {
		t.Errorf("expected action 'eliminate', got %q", move.Action)
	}
	...
	if !strings.Contains(move.Explanation, "all candidates in color") {
		t.Errorf("expected an all-candidates-in-color explanation, got %q", move.Explanation)
	}
}
